...
use nix_sandbox::cache::{CacheMetadata, EnvironmentCache};
...
fn test_cache_creation_and_retrieval() {
    ...
    assert!(
        cache
            .get_cached_environment(&environment)
            .unwrap()
            .is_none()
    );
...
fn test_cache_invalidation_on_flake_change() {
    ...
    fs::write(
        &flake_path,
        "{ description = \"Test flake v2 - modified\"; }",
    )
    .unwrap();
    ...
    assert!(
        cache
            .get_cached_environment(&modified_environment)
            .unwrap()
            .is_none()
    );
}
...
fn test_cache_cleanup_stale_entries() {
    ...
    assert!(
        ...
        "Cleanup shouldn't remove new caches"
    );
...
"#,
    ...
    let fake_store_path = temp_dir.path().to_string_lossy().to_string();
    let store_paths = vec![fake_store_path.clone()];
    let env_vars = HashMap::from([("GREETING".to_string(), "Hello from devenv!".to_string())]);
    ...
    let handles: Vec<_> = (0..5)
        .map(|i| {
            ...
            thread::spawn(move || {
                ...
                let env_vars = HashMap::from([("THREAD_ID".to_string(), i.to_string())]);
